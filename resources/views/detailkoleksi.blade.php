                ...
                <div class="mt-6">
                    ...
                    <div class="space-y-4">
                        @forelse ($koleksi['stok_varian'] as $jenis => $ukuranStok)
                            @php $hasStock = collect($ukuranStok)->filter(fn($stok) => $stok > 0)->isNotEmpty(); @endphp
                            @if ($hasStock)
                                <div class="bg-white p-4 rounded-lg shadow-sm border">
                                    ...
                                    <div class="flex flex-wrap gap-x-6 gap-y-2 mt-2">
                                        @foreach ($ukuranStok as $ukuran => $stok)
                                            @if ($stok > 0)
                                                <div><span
                                                        class="font-medium uppercase">{{ $ukuran }}:</span><span
                                                        class="text-gray-600">{{ $stok }} pcs</span></div>
                                            @endif
                                        @endforeach
                                    </div>
                                </div>
